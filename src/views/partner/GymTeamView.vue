    ...
    <div v-if="deleteRoleModal.show" class="modal-overlay" @click.self="deleteRoleModal.show = false">
      <div class="modal modal-sm">
        ...
        <div class="modal-body">
          <p class="confirm-text">Delete the <strong>{{ deleteRoleModal.label }}</strong> role? Staff with this role will need to be reassigned.</p>
        </div>
        <div class="modal-footer">
...
async function fetchMembers() {
  loading.value = true
  try {
    const res = await fetch('http://localhost:8000/api/v1/partner/team', {
...
async function fetchRoles() {
  const res = await fetch('http://localhost:8000/api/v1/partner/roles', {
    headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
  })
...
const assignableRoles = computed(() =>
  roles.value.filter(r => r.name !== 'gym_partner')
)
...
async function toggleActive(m: Member) {
  await fetch(`http://localhost:8000/api/v1/partner/team/${m.id}/toggle-active`, {
    method:  'PATCH',
    headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
  })
  fetchMembers()
...
async function submitRemove() {
  ...
  try {
    await fetch(`http://localhost:8000/api/v1/partner/team/${removeModal.value.id}`, {
      method:  'DELETE',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
...
async function submitResetTemp() {
  ...
  try {
    const res  = await fetch(`http://localhost:8000/api/v1/partner/team/${resetModal.value.id}/reset-temp`, {
      method:  'POST',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
    const data = await res.json()
...
const previewSlug = computed(() =>
  'gym_' + newRoleLabel.value.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')
)
...
async function submitDeleteRole() {
  ...
  try {
    await fetch(`http://localhost:8000/api/v1/partner/roles/${deleteRoleModal.value.id}`, {
      method:  'DELETE',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
...
.tbl tbody tr:hover { background: #fdfaff; }
...
.role-card:hover { border-color: #B8F0C0; box-shadow: 0 4px 12px rgba(76,217,100,0.08); }
.role-card-icon {
