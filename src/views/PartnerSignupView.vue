...
function onLicenseFile(e: Event) {
  ...
  if (file.type.startsWith('image/')) {
    ...
    reader.onload = () => { licensePreview.value = reader.result as string }
...
const cities = [
  'Addis Ababa', 'Hawassa', 'Adama (Nazret)', 'Dire Dawa',
  'Bahir Dar', 'Mekelle', 'Gondar', 'Jimma', 'Dessie', 'Jijiga',
]

const subCitiesMap: Record<string, string[]> = {
  'Addis Ababa': [
    'Arada', 'Addis Ketema', 'Akaki Kality', 'Bole', 'Gullele',
...
const step3Valid = computed(() => {
  ...
  return (
    ...
    cap > 0 &&
    (weekendClosed.value || (weekendOpen.value && weekendClose.value && weekendClose.value > weekendOpen.value))
  )
})
...
async function handleSubmit() {
  ...
  try {
    ...
    const res = await fetch('http://localhost:8000/api/v1/auth/register/partner', {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      const firstErr = data.errors ? Object.values(data.errors as Record<string, string[]>)[0][0] : null
  ...
  <div class="signup-page">
    <div class="signup-card">
      ...
      <div v-if="step === 1" class="card-body">
        ...
        <div class="fields">
          ...
          <div class="field-group">
            ...
            <input v-model="facilityName" type="text" class="input" placeholder="e.g. Sheraton Addis Fitness Center" />
          </div>
            ...
            <div class="category-grid">
              ...
              >
                <span class="cat-check">
                  <svg v-if="selectedCategories.includes(cat.id)" width="11" height="11" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"/></svg>
                </span>
                {{ cat.label }}
              </button>
            </div>
          ...
          <div class="field-group">
            ...
            <input v-model="contactPerson" type="text" class="input" placeholder="Owner or General Manager full name" />
          </div>
          ...
          <div class="field-row">
            ...
            <div class="field-group">
              ...
              <input v-model="contactEmail" type="email" class="input" placeholder="info@yourfacility.et" />
            </div>
          </div>

          <!-- TIN -->
          <div class="field-group">
            <label class="field-label">TIN (Taxpayer Identification Number) <span class="req">*</span></label>
            <input v-model="tinNumber" type="text" class="input" placeholder="e.g. 0021234567" maxlength="20" />
            <p class="field-hint">Your 10-digit tax registration number issued by ERCA</p>
          </div>

          <!-- Business License Upload -->
          <div class="field-group">
            <label class="field-label">Business License <span class="optional">(PDF or photo, max 10 MB)</span></label>
            ...
            <label v-else class="upload-zone">
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" class="upload-input" @change="onLicenseFile" />
      ...
      <div v-else-if="step === 2" class="card-body">
        ...
        <div class="fields">
          ...
          <div class="field-row">
            <div class="field-group">
              ...
              <div class="select-wrap">
                ...
                <svg class="select-arrow" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </div>
            ...
            <div class="field-group">
              ...
              <div class="select-wrap">
                <select v-model="subCity" class="input select-input" :disabled="!availableSubCities.length">
                  <option value="">{{ availableSubCities.length ? 'Select sub-city' : 'Not applicable' }}</option>
          ...
          <div class="field-group">
            ...
            <input v-model="woreda" type="text" class="input" placeholder="e.g. Woreda 03, House No. 247" />
            <p class="field-hint">Used for physical verification and local mapping</p>
          </div>

          <!-- Landmark -->
          <div class="field-group">
            <label class="field-label">Specific Landmark / Neighborhood <span class="optional">(optional)</span></label>
            <input v-model="landmark" type="text" class="input" placeholder='e.g. "Behind Edna Mall, next to Commercial Bank"' />
            <p class="field-hint">Help members find you easily on foot or by taxi</p>
          </div>

          <!-- Maps link -->
          <div class="field-group">
            <label class="field-label">Google Maps Link / GPS Coordinates <span class="optional">(optional)</span></label>
            <input v-model="mapsLink" type="text" class="input" placeholder="https://maps.google.com/... or 9.0123,38.7456" />
            <p class="field-hint">Used to plot your facility on the employee mobile directory map</p>
          </div>
      ...
      <div v-else-if="step === 3" class="card-body">
        ...
        <p class="subtext">Set your opening hours and how many FitAccess members you can accommodate.</p>
        ...
        <div class="fields">
          ...
          <div class="field-group">
            ...
            <div class="hours-block">
              ...
              <p v-if="weekdayOpen && weekdayClose && weekdayClose <= weekdayOpen" class="field-hint error-hint">
            ...
            <div class="hours-block">
              ...
              <p v-if="!weekendClosed && weekendOpen && weekendClose && weekendClose <= weekendOpen" class="field-hint error-hint">
                Closing time must be after opening time.
              </p>
            </div>
          </div>
          ...
          <div class="field-group">
            ...
            <p class="field-hint">How many FitAccess members can your facility comfortably accommodate per hour?</p>
          </div>

          <!-- Amenities (optional) -->
          <div class="field-group">
            <label class="field-label">Available Amenities <span class="optional">(optional — select all that apply)</span></label>
        ...
        <div class="actions">
          <button class="btn-next" :class="{ active: step3Valid }" :disabled="!step3Valid" @click="next">
            Continue
            <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </button>
        </div>
      ...
      <div v-else class="card-body">
        ...
        <p class="subtext">Set a secure password for <strong>{{ contactEmail }}</strong></p>
        ...
        <div class="fields">
          ...
          <div class="field-group">
            ...
            <div v-if="password" class="strength-meter">
              <div class="strength-bars">
                <div v-for="i in 4" :key="i" class="strength-bar" :class="passStrength >= i ? `bar-${passStrength}` : ''"></div>
              </div>
          ...
          <div class="field-group">
            ...
            <p v-if="confirmPassword && !passwordMatch" class="field-hint error-hint">Passwords do not match.</p>
          </div>
...
.progress-step.active .step-dot { background: #16a34a; color: #fff; box-shadow: 0 0 0 4px rgba(22,163,74,0.15); }
.progress-step.done  .step-dot  { background: #16a34a; color: #fff; }
.step-label { font-size: 0.72rem; color: #aaa; font-weight: 500; white-space: nowrap; transition: color 0.3s; }
...
.back-btn {
  ...
  padding: 0; margin-bottom: 20px; width: fit-content; transition: color 0.2s;
}
...
.cat-btn {
  ...
  text-align: left; font-size: 0.85rem; font-weight: 500; color: #444;
  transition: all 0.2s; font-family: inherit;
}
.cat-btn:hover { border-color: #16a34a; background: #f0fdf4; color: #166534; }
.cat-btn.selected { border-color: #16a34a; background: #f0fdf4; color: #166534; font-weight: 600; }
.cat-check {
  width: 18px; height: 18px; border-radius: 5px; flex-shrink: 0;
  border: 1.5px solid #ddd8d2; background: #fff;
  display: flex; align-items: center; justify-content: center;
  transition: all 0.2s; color: #16a34a;
}
...
.phone-wrap:focus-within { border-color: #16a34a; background: #fff; box-shadow: 0 0 0 3px rgba(22,163,74,0.08); }
.phone-prefix {
  display: flex; align-items: center; gap: 6px;
  padding: 0 14px; background: #f0ede8;
...
.upload-zone {
  ...
  border: 2px dashed #ddd8d2; border-radius: 12px;
  background: #f9f7f5; cursor: pointer;
  transition: border-color 0.2s, background 0.2s;
}
.upload-zone:hover { border-color: #16a34a; background: #f0fdf4; }
.upload-input { display: none; }
.upload-text  { font-size: 0.88rem; font-weight: 600; color: #555; margin: 0; }
.upload-hint  { font-size: 0.76rem; color: #999; margin: 0; }
...
.license-preview-inner { display: flex; align-items: center; gap: 12px; min-width: 0; }
.pdf-icon { flex-shrink: 0; }
.license-thumb { width: 44px; height: 44px; border-radius: 6px; object-fit: cover; flex-shrink: 0; }
.license-info  { min-width: 0; }
.license-name  { font-size: 0.85rem; font-weight: 600; color: #166534; margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.license-size  { font-size: 0.75rem; color: #16a34a; margin: 0; }
.license-remove {
...
.hours-day-label { font-size: 0.85rem; font-weight: 600; color: #333; }
.closed-toggle {
...
.hours-field { display: flex; flex-direction: column; gap: 5px; flex: 1; }
.time-label  { font-size: 0.75rem; color: #888; font-weight: 500; }
.time-input  { padding: 10px 14px; font-size: 0.9rem; }
.hours-dash  { font-size: 1.2rem; color: #ccc; padding-bottom: 10px; flex-shrink: 0; }
.closed-banner {
...
.capacity-wrap {
  ...
  background: #f9f7f5; transition: border-color 0.2s, box-shadow 0.2s;
}
.capacity-wrap:focus-within { border-color: #16a34a; background: #fff; box-shadow: 0 0 0 3px rgba(22,163,74,0.08); }
.capacity-input { border: none !important; border-radius: 0 !important; box-shadow: none !important; background: transparent !important; flex: 1; width: auto; }
.capacity-unit {
  padding: 0 16px; font-size: 0.82rem; font-weight: 600;
  color: #888; background: #f0ede8; border-left: 1.5px solid #ddd8d2;
...
.amenity-btn {
  padding: 9px 10px; background: #f9f7f5;
  border: 1.5px solid #ddd8d2; border-radius: 8px;
...
.rule-ok, .rule-fail { font-size: 0.75rem; }
...
.summary-chip {
  ...
  border: 1.5px solid #bbf7d0; border-radius: 12px; margin-bottom: 24px;
}
.summary-avatar {
...
.btn-next {
  ...
  font-size: 0.97rem; font-weight: 700; border: none;
  background: #ccc; color: #888; cursor: not-allowed;
  display: flex; align-items: center; justify-content: center; gap: 8px;
  transition: all 0.2s; font-family: inherit;
}
.btn-next.active { background: #1b3a6b; color: #fff; cursor: pointer; }
.btn-next.active:hover { background: #162f58; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(27,58,107,0.25); }
.btn-partner.active { background: #16a34a; }
.btn-partner.active:hover { background: #15803d; box-shadow: 0 6px 20px rgba(22,163,74,0.3); }

.review-notice {
  display: flex; align-items: flex-start; gap: 8px;
  font-size: 0.82rem; color: #888; background: #f9f7f5;
...
.signin-link a { color: #16a34a; font-weight: 600; text-decoration: none; }
