...
onMounted(async () => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/public/companies`, {
...
const step2Valid = computed(() => companyId.value !== null && staffId.value.trim().length > 0)
...
async function handleSubmit() {
  ...
  error.value = ''
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/auth/register/employee`, {
      method: 'POST',
  ...
  <div class="signup-page">
    <div class="signup-card">
        ...
        <div class="fields">
          <div class="field-group" ref="companyDropRef" style="position: relative">
            ...
            <div v-if="showCompanyDrop && !companyId" class="company-drop">
              ...
              <div v-else-if="filteredCompanies.length === 0" class="drop-empty">
                No companies match "{{ companySearch }}"
              </div>
              <div v-else>
                <div v-if="!companySearch" class="drop-header">
            ...
            <p v-if="!companyId" class="field-hint">
              Only active companies registered in FitAccess appear here.
            </p>
          </div>

          <div class="field-group">
            <label class="field-label"
              >Employee ID / Staff ID Number <span class="req">*</span></label
            >
            <input
              :value="staffId"
      ...
      <div v-else class="card-body">
          ...
          <div class="review-section">
            ...
            <div class="review-row">
              ...
              ><span class="review-val"
                >{{ firstName }} {{ fathersName }} {{ grandfathersName }}</span
              >
            </div>
            <div class="review-row">
              <span class="review-key">Email</span><span class="review-val">{{ email }}</span>
            </div>
            <div class="review-row">
              <span class="review-key">Phone</span><span class="review-val">+251 {{ phone }}</span>
            </div>
          </div>
          <div class="review-section">
            ...
            <div class="review-row">
              ...
              ><span class="review-val">{{ companyName }}</span>
            </div>
            <div class="review-row">
              <span class="review-key">Staff ID</span
              ><span class="review-val mono">{{ staffId }}</span>
            </div>
            <div v-if="jobPosition" class="review-row">
              <span class="review-key">Position</span
              ><span class="review-val">{{ jobPosition }}</span>
            </div>
            <div v-if="department" class="review-row">
              <span class="review-key">Department</span
              ><span class="review-val">{{ department }}</span>
            </div>
            <div v-if="branch" class="review-row">
              <span class="review-key">Branch</span><span class="review-val">{{ branch }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="field-group" style="margin-top: 16px">
          <label class="field-label"
            >Request Note to HR <span class="optional">(optional)</span></label
          >
          <textarea
            v-model="requestNote"
    ...
    <footer class="signup-footer">
      <p class="footer-notice">
        Protected by FitAccess Ethiopia &nbsp;·&nbsp; <a href="#">Privacy Policy</a>
      </p>
    </footer>
  </div>
</template>
...
.progress-step.active .step-dot {
  background: #4cd964;
  color: #fff;
  box-shadow: 0 0 0 4px rgba(224, 56, 106, 0.15);
}
...
.btn-next.active:hover {
  ...
  box-shadow: 0 6px 20px rgba(224, 56, 106, 0.35);
}
...
.plan-card:hover {
  border-color: #4cd964;
  background: #fff;
  box-shadow: 0 4px 14px rgba(224, 56, 106, 0.12);
}
.plan-card--selected {
  border-color: #4cd964;
  background: #fff8fa;
  box-shadow: 0 4px 14px rgba(224, 56, 106, 0.15);
}
