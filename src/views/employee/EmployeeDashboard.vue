      ...
      <div class="top-row">
        ...
        <div class="card profile-card">
          <div class="profile-avatar">
            <img v-if="data.profile.photo_path"
              :src="`http://localhost:8000/storage/${data.profile.photo_path}`"
          ...
          <div class="profile-info">
            ...
            <div class="profile-contacts">
              <span v-if="data.profile.email" class="contact-item">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16v16H4z"/><path d="M4 4l8 8 8-8"/></svg>
                {{ data.profile.email }}
              </span>
              <span v-if="data.profile.phone" class="contact-item">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6.29 6.29l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                {{ data.profile.phone }}
              </span>
            </div>
          </div>
        ...
        <div class="card pass-card" :class="`pass-card--${data.pass.plan_key}`">
          ...
          <div class="pass-footer">
            <div>
              ...
              <p class="pass-meta-val">{{ data.pass.plan_key === 'platinum' ? 'Premium Access' : data.pass.plan_key === 'basic_plus' ? 'Plus Access' : 'Basic Access' }}</p>
            </div>
      ...
      <div class="bottom-row">
        ...
        <div class="card section-card">
          ...
          <div v-if="data.recent_checkins.length === 0" class="empty-state">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" stroke-width="1.5"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
          ...
          <ul v-else class="checkin-list">
            <li v-for="c in data.recent_checkins" :key="c.id" class="checkin-item">
              ...
              <div class="checkin-right">
                ...
                <span class="tier-badge tier-badge--{{ c.gym_tier }}">{{ tierLabel(c.gym_tier) }}</span>
              </div>
            </li>
          </ul>
        </div><!-- /recent check-ins -->
        ...
        <div class="card section-card">
          ...
          <div v-if="data.accessible_gyms.length === 0" class="empty-state">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" stroke-width="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
            ...
            <li v-for="g in data.accessible_gyms" :key="g.id" class="gym-item">
              <div class="gym-left">
                ...
                <div>
                  ...
                  <p class="gym-addr">{{ g.sub_city }}<span v-if="g.city">, {{ g.city }}</span></p>
                </div>
              </div>
...
function statusLabel(s: string): string {
  return s === 'active' ? 'Active' : s === 'suspended' ? 'Suspended' : s === 'inactive' ? 'Inactive' : s
}
...
.profile-meta { font-size: 0.82rem; color: #64748b; margin: 0 0 3px; }
.profile-company { font-size: 0.8rem; color: #94a3b8; margin: 0 0 12px; }
.dot { margin: 0 4px; }
.profile-contacts { display: flex; flex-direction: column; gap: 5px; }
.contact-item {
...
.pass-label { font-size: 0.65rem; letter-spacing: 0.15em; color: rgba(255,255,255,0.5); margin: 0 0 4px; }
.pass-plan { font-size: 1.3rem; font-weight: 800; margin: 0; letter-spacing: -0.01em; }
.pass-badge {
  padding: 4px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600;
  text-transform: uppercase; letter-spacing: 0.05em; flex-shrink: 0;
}
.badge--active    { background: rgba(34,197,94,0.2); color: #86efac; border: 1px solid rgba(134,239,172,0.3); }
.badge--suspended { background: rgba(251,191,36,0.2); color: #fde68a; border: 1px solid rgba(253,230,138,0.3); }
.badge--inactive  { background: rgba(239,68,68,0.2); color: #fca5a5; border: 1px solid rgba(252,165,165,0.3); }

.pass-fan { margin-bottom: 20px; }
.fan-label  { font-size: 0.68rem; color: rgba(255,255,255,0.4); letter-spacing: 0.1em; margin: 0 0 4px; }
.fan-number {
...
.pass-meta-label { font-size: 0.65rem; color: rgba(255,255,255,0.4); letter-spacing: 0.08em; margin: 0 0 3px; }
.pass-meta-val   { font-size: 0.82rem; font-weight: 600; color: rgba(255,255,255,0.85); margin: 0; }
...
.section-count  { font-size: 0.78rem; color: #94a3b8; }
...
.checkin-left  { display: flex; align-items: center; gap: 12px; min-width: 0; }
.checkin-right { display: flex; flex-direction: column; align-items: flex-end; gap: 3px; flex-shrink: 0; }
.checkin-dot   { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.checkin-gym   { font-size: 0.85rem; font-weight: 600; color: #1e293b; margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.checkin-loc   { font-size: 0.75rem; color: #94a3b8; margin: 0; }
.checkin-date  { font-size: 0.75rem; color: #64748b; margin: 0; }
.checkin-dur   { font-size: 0.72rem; color: #94a3b8; margin: 0; }

/* Gym list */
.gym-list { list-style: none; margin: 0; padding: 0; max-height: 420px; overflow-y: auto; }
.gym-item {
  display: flex; align-items: center; justify-content: space-between;
...
.gym-left { display: flex; align-items: center; gap: 12px; min-width: 0; }
.gym-tier-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.gym-name { font-size: 0.85rem; font-weight: 600; color: #1e293b; margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
