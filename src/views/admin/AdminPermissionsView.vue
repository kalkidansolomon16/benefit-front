      ...
      <div class="role-legend">
        <div
          ...
          :style="{
            ...
            borderColor: PALETTE[idx % PALETTE.length].accent + '66',
            color: PALETTE[idx % PALETTE.length].text,
          }"
        >
          <span class="role-dot" :style="{ background: PALETTE[idx % PALETTE.length].accent }"></span>
          {{ role.label }}
        </div>
      </div>
        ...
        <div class="perm-row perm-head">
          ...
          <div v-for="(role, rIdx) in editableRoles" :key="role.key" class="perm-role-col">
            <span
              ...
              :style="{
                background: PALETTE[rIdx % PALETTE.length].bg,
                color: PALETTE[rIdx % PALETTE.length].text,
              }"
            >{{ role.label }}</span>
          </div>
        </div>
        ...
        >
          ...
          <div v-for="(role, rIdx) in editableRoles" :key="role.key" class="perm-role-col">
            <button
              ...
              :style="hasRolePerm(role.key, perm.name) ? {
                background: PALETTE[rIdx % PALETTE.length].accent,
                borderColor: PALETTE[rIdx % PALETTE.length].accent,
                color: '#fff',
              } : {}"
...
async function fetchPermissions() {
  loading.value = true
  try {
    const res = await fetch('http://localhost:8000/api/v1/admin/team/permissions', {
...
async function savePermissions() {
  ...
  try {
    for (const role of editableRoles.value) {
      await fetch(`http://localhost:8000/api/v1/admin/team/permissions/roles/${role.key}`, {
        method:  'PUT',
