  ...
  <div class="page">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage admin sub-users and roles.</p>
      </div>
    ...
    <div class="section-header">
      <div>
        ...
        <p class="page-sub">Create custom roles that can be assigned to team members.</p>
      </div>
      ...
      <div class="modal">
        ...
        <form @submit.prevent="submitCreate" class="modal-body">
          <div class="field">
            ...
            <input v-model="createForm.name" required placeholder="John Doe" />
          </div>
          <div class="field">
            <label>Email Address</label>
            <input type="email" v-model="createForm.email" required placeholder="john@example.com" />
          </div>
          <div class="field">
            <label>Role</label>
    ...
    <div v-if="tempPasswordModal.show" class="modal-overlay">
      <div class="modal">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="alert-info">
            ...
            Share this temporary password with <strong>{{ tempPasswordModal.name }}</strong>. It will not be shown again.
          </div>

          <div class="pw-reveal">
            <span class="pw-text">{{ tempPasswordModal.password }}</span>
            <button class="copy-btn" @click="copyTempPw">{{ copiedPw ? '✓ Copied!' : 'Copy' }}</button>
          </div>

          <p class="pw-note">The user must activate their account and reset their password on first login.</p>
        </div>
        <div class="modal-footer">
          <button class="btn-primary" @click="tempPasswordModal.show = false">I've Saved It</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="removeModal.show" class="modal-overlay" @click.self="removeModal.show = false">
      <div class="modal modal-sm">
        ...
        <div class="modal-body">
          <p class="confirm-text">Are you sure you want to remove <strong>{{ removeModal.name }}</strong>? This cannot be undone.</p>
        </div>
        <div class="modal-footer">
    ...
    <!-- ── Reset temp password confirm modal ───────────────── -->
...
async function fetchMembers() {
  loading.value = true
  try {
    const res = await fetch('http://localhost:8000/api/v1/admin/team', {
...
onMounted(() => { fetchMembers(); fetchRoles() })

// Roles available to assign (exclude the primary admin role)
const assignableRoles = computed(() =>
  roles.value.filter(r => r.name !== 'fitaccess_admin')
)
...
const tempPasswordModal = ref({ show: false, name: '', password: '' })
...
async function submitCreate() {
  ...
  try {
    ...
    tempPasswordModal.value = { show: true, name: data.user.name, password: data.temp_password }
    fetchMembers()
  } catch (e: any) {
...
function copyTempPw() {
  navigator.clipboard.writeText(tempPasswordModal.value.password).catch(() => {})
  copiedPw.value = true
...
async function toggleActive(m: Member) {
  await fetch(`http://localhost:8000/api/v1/admin/team/${m.id}/toggle-active`, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
  })
  fetchMembers()
...
async function submitRemove() {
  ...
  try {
    await fetch(`http://localhost:8000/api/v1/admin/team/${removeModal.value.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
...
async function submitResetTemp() {
  ...
  try {
    const res  = await fetch(`http://localhost:8000/api/v1/admin/team/${resetModal.value.id}/reset-temp`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
    const data = await res.json()
    resetModal.value.show   = false
    tempPasswordModal.value = { show: true, name: resetModal.value.name, password: data.temp_password }
    fetchMembers()
...
const previewSlug = computed(() =>
  'admin_' + newRoleLabel.value.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')
)
...
async function submitDeleteRole() {
  ...
  try {
    await fetch(`http://localhost:8000/api/v1/admin/roles/${deleteRoleModal.value.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
...
.role-badge.admin_finance { background: #EBFAEE; color: #1A9E38; }
.role-badge.admin_support { background: #EBFAEE; color: #1A9E38; }
...
.pw-text {
  ...
  letter-spacing: 0.1em; color: #1A9E38;
}
.copy-btn {
  background: #EBFAEE; border: 1.5px solid #B8F0C0;
...
.section-title {
  font-size: 1.15rem; font-weight: 700; color: #0f172a; margin: 0 0 4px;
}
...
.role-card:hover { border-color: #B8F0C0; box-shadow: 0 4px 12px rgba(99,102,241,0.08); }
