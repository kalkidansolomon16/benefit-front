      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="alert-info">
            ...
            Share this temporary password with <strong>{{ tempPasswordModal.name }}</strong
          ...
          <p class="pw-note">
            The user must activate their account and reset their password on first login.
          </p>
        </div>
        <div class="modal-footer">
...
async function fetchMembers() {
  loading.value = true
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/admin/team`, {
...
async function fetchRoles() {
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/admin/roles`, {
    headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
  })
...
const assignableRoles = computed(() => roles.value.filter((r) => r.name !== 'fitaccess_admin'))
...
async function toggleActive(m: Member) {
  await fetch(`${import.meta.env.VITE_API_BASE_URL}/admin/team/${m.id}/toggle-active`, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
  })
  fetchMembers()
...
async function submitResetTemp() {
  ...
  try {
    const res = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/admin/team/${resetModal.value.id}/reset-temp`,
      {
        method: 'POST',
        headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
      }
...
async function submitDeleteRole() {
  ...
  try {
    await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/admin/roles/${deleteRoleModal.value.id}`,
      {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
      }
...
.role-badge.admin_finance {
  background: #ebfaee;
  color: #1a9e38;
}
.role-badge.admin_support {
  background: #ebfaee;
  color: #1a9e38;
}
