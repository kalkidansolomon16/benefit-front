...
async function fetchMembers() {
  loading.value = true
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/team`, {
...
async function submitRemove() {
  ...
  try {
    await fetch(`${import.meta.env.VITE_API_BASE_URL}/hr/team/${removeModal.value.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
    })
...
async function submitResetTemp() {
  ...
  try {
    const res = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/hr/team/${resetModal.value.id}/reset-temp`,
      {
        method: 'POST',
        headers: { Authorization: `Bearer ${auth.token}`, Accept: 'application/json' },
      }
